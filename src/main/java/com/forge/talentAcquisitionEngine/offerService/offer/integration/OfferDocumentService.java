...
public class OfferDocumentService {
    ...
    public byte[] generateOfferPdf(Offer offer) {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            throw new RuntimeException("Failed to generate offer PDF", e);
        }
    }

    private String buildOfferHtml(Offer offer) {
        String candidateName = offer.getApplication().getCandidate().getFirstName()
                + " " + offer.getApplication().getCandidate().getLastName();

        // Use active template if available, otherwise use default
        String templateContent = offerTemplateRepository.findByActiveTrue()
        ...
        return templateContent
                ...
                .replace("{{bonus}}", offer.getBonus() != null ? offer.getBonus().toPlainString() : "0")
                .replace("{{equity}}", offer.getEquity() != null ? offer.getEquity().toPlainString() : "0")
